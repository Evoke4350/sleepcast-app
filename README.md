...
1. `NightAudio.mm` must be added to the Xcode target — new files in
   `ios/SleepcastApp/` are not picked up automatically by an existing
   `.xcodeproj`.
2. `#import "SleepcastApp-Swift.h"` is generated by Xcode from the Swift
