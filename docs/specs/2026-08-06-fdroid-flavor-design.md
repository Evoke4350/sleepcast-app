...
- **Per-flavor JS bundle env:** the RN gradle plugin's embedded-bundle task (`createBundleFoss*JsAndAssets`, a `BundleHermesCTask`) must run Metro with `SLEEPCAST_FOSS=1` so the resolver swaps. That task is a plain `DefaultTask` shelling out via `ExecOperations.exec` — it exposes **no** settable per-task env (an `it.environment(...)` hook is a silent no-op). It **does** expose `nodeExecutableAndArgs`, prepended to the bundle command line, so wrap it: `t.nodeExecutableAndArgs.set(["env", "SLEEPCAST_FOSS=1"] + base)` makes the command literally `env SLEEPCAST_FOSS=1 node cli.js bundle …` — the swap fires with a plain `./gradlew assembleFossRelease`, no shell prefix (`env` is on every Linux/macOS host). A `doFirst` clears the task's `jsBundleDir`/`resourcesDir` first, because Metro's `--assets-dest` never deletes orphaned assets (a stale `model.onnx` from a prior build would otherwise be packaged).
- **Fail-closed guard:** the foss bundle task's `doLast` recurses `jsBundleDir`/`resourcesDir` and throws a `GradleException` if any `.onnx` > 1 MB is present — so if the swap ever silently fails to apply, the build **fails loudly** instead of shipping the 23 MB model (which AGP renames, e.g. `res/yt.onnx`, defeating a filename grep). This makes the "no model in the foss APK" guarantee enforced by the build, not by a human remembering a step. The `onnxruntime` patch-package/postinstall stays (harmless).
